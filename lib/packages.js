...
export const PACKAGE_GROUPS = [
  ...
  {
    category: 'Testing (Select One)',
    type: 'list',
    id: 'testing',
    choices: [
      { name: 'None', value: 'none' },
      { name: 'Vitest + React Testing Library (Unit/Component)', value: 'vitest' },
      { name: 'Playwright (E2E)', value: 'playwright' },
      { name: 'Both (Vitest + Playwright)', value: 'both' },
    ],
    default: 'none',
    providerConfig: {
      vitest: {
        ...
        devInstallNextjs: ['@vitejs/plugin-react', 'vite-tsconfig-paths'],
        guidance: 'Place tests next to source files (`*.test.tsx`). Use `@testing-library/react` for component rendering and `@testing-library/user-event` for interactions. Note: async Server Components cannot be tested with Vitest — use E2E tests for those.',
      },
      playwright: {
        name: 'Playwright',
        ...
        guidance: 'Place E2E tests in `e2e/` or `tests/` directory. Configure `webServer` in `playwright.config.ts` to start the dev server automatically. Use `npx playwright test` to run and `npx playwright test --ui` for the interactive UI.',
      },
      both: {
        ...
        guidance: 'Unit tests: place next to source (`*.test.tsx`), run with `npx vitest`. E2E tests: place in `e2e/` directory, run with `npx playwright test`. Use Vitest for component logic and Playwright for full user flows.',
      },
    },
  },
  {
